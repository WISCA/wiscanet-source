...
void rxMsgEnodeLogAck(int sock, cMsgEnodeLogAck_t *pload, int size) 
{
    ...
    // log copy
    sprintf(cbuf, "mkdir -p ../log/enode_%d; cd ../log/enode_%d; scp %s@%s:wdemo/run/enode/log/wisca_log.tgz .", nodeId, nodeId, userName, sysConf[nodeId].ipaddr);
    system(cbuf);
...
printf("cnfFName = %s\n", cnfFName);
        ...
        // install matlab file
        sprintf(cbuf, "cd ../../usr; tar cfz umat.tgz mat; scp umat.tgz %s@%s:wdemo/run/enode", userName, sysConf[cIdx].ipaddr); 
        system(cbuf);
...
void logAnalysis()
{
    ...
    while((cIdx = getActiveNodeIdx(cIdx)) != -1) {
        if(sysConf[cIdx].usrFlag == 1) {
            if(strcmp("NULL", sysConf[cIdx].mLogFile) == 0) continue;
            sprintf(cbuf, "cp ../../usr/mat/%s/%s.m ../log/enode_%d", sysConf[cIdx].matDir, sysConf[cIdx].mLogFile, cIdx);
            system(cbuf);
        }
    }
...
void remoteEnodeStop()
{
    ...
    while(fgets(ibuf, 128, fp) != NULL) {
        sscanf(ibuf, "%s", ipaddr);
        sprintf(cmdBuf, "ssh %s@%s 'cd wdemo/run/enode/tools; appkill %s; appkill %s; appkill %s'", userName, ipaddr, "enode", "MATLAB", "uControl");
        printf("%s\n", cmdBuf);
        system(cmdBuf);
    }
}
...
void remoteEnodeStart()
{
    ...
    while(fgets(ibuf, 128, fp) != NULL) {
        sscanf(ibuf, "%s", ipaddr);
        sprintf(cmdBuf, "ssh %s@%s 'cd wdemo/run/enode/bin; nohup ./enode  &> ../conlog/enode.log < /dev/null &'", userName, ipaddr);
printf("%s\n", cmdBuf);
        system(cmdBuf);
    }
}
...
void rxMsgUsrCmd(int sock, cMsgUsrCmd_t *pload, int size) 
{
    ...
    switch(pload->usrCmd) {
        case CMD_PRT_STATUS : break;
        case CMD_DL_MATLAB : downloadMatFiles(); break;
        case CMD_EX_MATLAB : enodeRunReq(); break;
        case CMD_STOP_MATLAB : enodeStopReq(); break;
        case CMD_CLT_LOG : enodeLogReq(); break;
        case CMD_LOG_ANAL : logAnalysis(); break;
        case (CMD_DL_MATLAB | CMD_EX_MATLAB) : dlNExecution(); break;
        default : printf("unkonw command\n");         
    }
}
