...
void rxMsgEnodeRunReq(cMsgEnodeRunReq_t *pload) {
	...
	if (uCtrlPid == 0) {  // child
		...
		sprintf(
		    cbuf,
		    "sudo ./uControl --args %s --opmode %s --macmode %s --tslot %d --nsamps %d --rate %f --subdev %s --freq %f --txgain %f --rxgain %f --bw %f --channels %s --ant %s &> ../conlog/usrpLog",
		    cfg.devAddr, (cfg.opMode == OPMODE_TX_RX) ? "TX/RX" : (cfg.opMode == OPMODE_RX) ? "RX" : "TX", cfg.macMode,
		    cfg.tSlot, cfg.nsamps, cfg.rate, cfg.subdev, cfg.freq, cfg.txgain, cfg.rxgain, cfg.bw, cfg.channels,
		    cfg.antennas);
#else
		sprintf(
		    cbuf,
		    "./uControl --args %s --opmode %s --macmode %s --tslot %d --nsamps %d --rate %f --subdev %s --freq %f --txgain %f --rxgain %f --bw %f --channels %s --ant %s &> ../conlog/usrpLog",
		    cfg.devAddr, (cfg.opMode == OPMODE_TX_RX) ? "TX/RX" : (cfg.opMode == OPMODE_RX) ? "RX" : "TX", cfg.macMode,
		    cfg.tSlot, cfg.nsamps, cfg.rate, cfg.subdev, cfg.freq, cfg.txgain, cfg.rxgain, cfg.bw, cfg.channels,
		    cfg.antennas);
#endif
	...
	if (matPid == 0) {
		if (cfg.lang == LANG_PYTHON) {
			...
			if (strcmp(cfg.macMode, "UMAC") == 0) {
				...
				sprintf(cbuf, "cd ../mat; mkdir -p ../log; cd %s; python -c %s < /dev/null | tee %s", cfg.matDir,
				        cfg.mTopFile, logFn);
			}
			printf("%s\n", cbuf);
			system(cbuf);
			...
			if (strcmp(cfg.macMode, "UMAC") == 0) {
				sprintf(cbuf,
				        "cd ../mat; mkdir -p ../log; cd %s; matlab -nodisplay -r %s\\(%ld\\) < /dev/null | tee %s",
				        cfg.matDir, cfg.mTopFile, start_time, logFn);
			} else {
				sprintf(cbuf, "cd ../mat; mkdir -p ../log; cd %s; matlab -nodisplay -r %s < /dev/null | tee %s",
				        cfg.matDir, cfg.mTopFile, logFn);
			}
			printf("%s\n", cbuf);
			system(cbuf);
