...
void rxMsgEnodeRunReq(cMsgEnodeRunReq_t *pload) {
    ...
    if (uCtrlPid == 0) {  // child
        ...
        sprintf(
                ...
                cfg.devAddr, (cfg.opMode == OPMODE_TX_RX) ? "TX/RX" : (cfg.opMode == OPMODE_RX) ? "RX" : "TX", cfg.nsamps, cfg.rate, cfg.subdev, cfg.freq, cfg.txgain, cfg.rxgain, cfg.bw, cfg.channels,
                cfg.antennas);
#else
        sprintf(
                cbuf,
