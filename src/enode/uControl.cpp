...
void recv_UMAC_worker(uhd::usrp::multi_usrp::sptr usrp, const std::string &cpu_format, const std::string &wire_format,
	...
	while (1) {
		...
		*/
		...
		int count = 0, txTotal = 0, txUnit = 1000, txLen = 0, txResult = 0;
		while (txTotal < txSamps) {
			...
			usleep(120);  // intentional delay for proper context switching
...
void recv_TDMA_worker(uhd::usrp::multi_usrp::sptr usrp, const std::string &cpu_format, const std::string &wire_format,
	...
	while (1) {
		...
		time_now = usrp->get_time_now(0);
		if (cslot == 0)
			rxTime = last_pps + uhd::time_spec_t(1.0);  // next pps
		else
			rxTime = rxTime + uhd::time_spec_t(sched_unit);
...
int synch_to_gps(uhd::usrp::multi_usrp::sptr usrp) {
	...
	if (std::find(sensor_names.begin(), sensor_names.end(), "ref_locked") != sensor_names.end()) {
		...
		for (int i = 0; i < 30 and not ref_locked; i++) {
			...
			if (not ref_locked) {
				std::cout << "." << std::flush;
				boost::this_thread::sleep(boost::posix_time::seconds{static_cast<long>(1)});
			}
		}
		if (ref_locked) {
			std::cout << "LOCKED" << std::endl;
...
void transmit_UMAC_worker(uhd::usrp::multi_usrp::sptr usrp, size_t total_num_samps, size_t tslot) {
	...
	while (1) {
		...
		time_now = usrp->get_time_now(0);
		start_time = (double *)(&buff[0] + (rLen / sizeof(std::complex<float>)) - 1);
		...
		while (txBufLoc < rSamLen) {
			if ((rSamLen - txBufLoc) > txUnit)
				txLen = txUnit;
			else
				txLen = (rSamLen - txBufLoc);
...
void transmit_TDMA_worker(uhd::usrp::multi_usrp::sptr usrp, size_t total_num_samps, size_t tslot) {
	...
	while (1) {
		...
		*/
		...
		printf("last_pps=%f, time_now=%f, txTime=%f\n", last_pps.get_real_secs(), time_now.get_real_secs(),
		       txTime.get_real_secs());
...
int UHD_SAFE_MAIN(int argc, char *argv[]) {
	...
	desc.add_options()("help", "help message")("args", po::value<std::string>(&args)->default_value(""),
